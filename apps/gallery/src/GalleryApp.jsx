      ...
      <main id="collection">
        <section className="collection" aria-labelledby="collection-title">
          ...
          {content.entries.map((entry) => (
            <article className="entry-card" key={entry.number}>
              <div className="entry-art" aria-hidden="true"><span>{entry.number}</span><i>{entry.number === '002' ? 'फूलबारी' : 'निवेदन'}</i></div>
              <div className="entry-copy">
                <p className="status"><span className="status-dot" aria-hidden="true" />{entry.statusEn} · <span lang="ne">{entry.statusNe}</span></p>
